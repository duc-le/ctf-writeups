...
Actually all those setups are not needed to by placed at **OnInitDialog**, but they must be called when the dialog has been created (maybe in some event handlers). We would still use the same approach in that case.
...
```
**0x2543** is the lowest address in the call stack. We might guess that other addresses are from the MFC static library. From **0x2543** we can easily navigate to **0x2480** where the **WM_SYSCOMMAND** handler starts. From there we just "Jump to xref" to locate the message map structure of the dialog at **0x211290**:
